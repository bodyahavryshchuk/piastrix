...
class Piastrix:
    ...
    def bill(self, shop_order_id, currency, amount, description):
        ...
        with urlopen(request) as response:
            ...
            if not data["result"]:
                ...
                flash('Error. Please try again: {}'.format(data['message']))
    ...
    def invoice(self, shop_order_id, currency, amount, description):
        ...
        with urlopen(request) as response:
            ...
            if not data["result"]:
                ...
                data = data["data"]
                loger.save_order(currency, amount, description, shop_order_id)
                return render_template('pay_way/invoice.html', method=data["method"], action=data["url"],
                                       form=data["data"])
    ...
    def _sign(self, **kwargs):
        ...
        myhash.update((":".join(data) + self.SECRET_KEY).encode("utf-8"))
